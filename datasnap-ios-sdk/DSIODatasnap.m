...
static DSIODatasnap* sharedInstance = nil;
static NSString* appInstalledEventType = @"app_installed";
@implementation DSIODatasnap
- (DSIODevice*)setDeviceAndReturn:(DSIODevice*)device
{
            ...
            andDSIOVendorProperties:(DSIOVendorProperties*)DSIOVendorProperties
{
    ...
    dispatch_once(&onceToken, ^{
        sharedInstance = [[self alloc] initWithApiKey:apiKey
                                  ...
                                  andDSIOVendorProperties:DSIOVendorProperties];
    });
}
...
- (id)initWithApiKey:(NSString*)apiKey
 ...
 andDSIOVendorProperties:(DSIOVendorProperties*)DSIOVendorProperties
{
    self.DSIOVendorProperties = DSIOVendorProperties;
    if (self = [self init]) {
...
{
    if (!self.DSIOVendorProperties) {
        return;
    }
    switch (self.DSIOVendorProperties.vendor) {
    case GIMBAL:
        self.gimbalClient = [[DSIOGimbalClient alloc] init];
        self.gimbalClient.gimbalApiKey = self.DSIOVendorProperties.gimbalApiKey;
...
- (void)trackEvent:(BaseEvent*)event
{
    ...
    [event.projectIds addObject:self.projectId];
    if (![event validate]) {
        NSLog(@"Mandatory event data missing. Please call DSIODatasnap.initialize before using the library");
    }
